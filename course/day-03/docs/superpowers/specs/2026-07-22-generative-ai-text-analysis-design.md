...
課程開始須完成 MCP 預檢：用指定測試文件成功轉出 Markdown，並確認可讀取指定課程資料夾。因 MCP 以使用者權限讀取本機檔案與可連線的網路資源，教材只允許指定資料夾與公開來源。
...
| 時段 | 主題 | 主要產出 |
| --- | --- | --- |
| 09:00–09:30 | 文本資料、來源與可追溯性 | 資料邊界理解與 MCP 預檢 |
| 09:30–10:30 | 指定網站爬取 | 原始檔與 crawl manifest |
| 10:30–12:00 | MarkItDown 解析與品質檢查 | Markdown 與檢查紀錄 |
...
5. **問答紀錄分析與改善**：從模擬紀錄找出高頻、未解與低證據問題，將每項問題歸因為補文件、改解析、改切分或 metadata、改檢索或改提示詞。
