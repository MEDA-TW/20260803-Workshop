...
依 [學生指南 05](../../STUDENT_GUIDE.md) 的短提示詞，學生用 Codex 從空白建立 `rag/app.py`，再以 `python -m streamlit run rag/app.py` 開啟自己建立的 Streamlit 問答機器人。先確認網頁成功載入、沒有 `ModuleNotFoundError`。使用向量檢索時，畫面必須顯示前三個 chunks、相似度與 provenance；另提供只讀取自己本機 `feedback/query_log.jsonl` 的「問答紀錄」頁。
